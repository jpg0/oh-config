...
let WARM_WHITE = adjustedColorTemp(1);
const WW_ROUNDING_HUE = 3;
const WW_ROUNDING_SAT = 0.1;

let isWarmWhite = (c) => {
    c = color.create(c);
    return  Math.abs(c.get('hsv.h') - WARM_WHITE.get('hsv.h')) < WW_ROUNDING_HUE &&
            Math.abs(c.get('hsv.s') - WARM_WHITE.get('hsv.s')) < WW_ROUNDING_SAT;
}
...
let determineColor = function (/*item*/ fromItem, time) {
    ...
    if(fromColor.toString() === "NULL") {
        fromColor = "0,0,0";
    }
    log.debug(`fromColor for ${fromItem.name} = ${fromColor}/${typeof(fromColor)}`);
    let to = color.create(toColor);
    let from = color.create(fromColor);
    log.debug(`merging color: ${from}->${to} for ${fromItem.name}`);
