...
class UpstairsHVACZone extends DuctedHVACZone {
    ...
    processPassiveHVAC(isActiveHVACEnabled) {
        ...
        if(isActiveHVACEnabled) {
            ...
            if(this.shouldHeatOrCoolNow() == 'cool') { //only need to close the door for cooling
                ...
                switch(coolOrHeat) {
                    ...
                    case null: {  //we're currently within passive bounds
                        if('min' in passiveBounds && this.outdoorTemperature < passiveBounds.min) {
                            ...
                            //good inside and out
                            log.debug("Ensure windows and shades open to get some air as it's good inside ({}°C) and out ({}°C)", this.currentTemperature, this.outdoorTemperature);
                            this.setWindowsOpen(true);
                            this.setShadesOpen(true);
                        }
                        break;
    ...
    get outdoorTemperature() {
        return items.getItem('Outside_Upstairs_Temperature_Temperature').state;
    }
...
let SLEEP_TIME_START = LocalTime.parse("21:45");
