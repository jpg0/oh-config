...
let isWarmWhite = function(c) {
    let warmWhiteHsb = constants.WARM_WHITE_COLOR;
    let wwhsb = warmWhiteHsb.toString().split(',');
    let hsb = c.split(',');

    if(hsb.length === 3) {
        if(hsb[0] == wwhsb[0] && hsb[1] == wwhsb[1]) {
            log.debug(`${c} is warm white`);
            return true;
        }
    }

    log.debug(`${c} is not warm white`);
    return false;
}
        ...
        rules.JSRule({
            ...
            execute: (args) => {
                ...
                if (receivedCommand.toString() === "ON") {
                    ...
                    // receivedCommand = items.getItem('vCurrent_Light_Color').rawState; //todo: update to get correct color
                }

                //to go from WW to RGB we need to first set the brightness to 0
                if(
                    isWarmWhite(currentState.toString()) && 
                    !isWarmWhite(receivedCommand.toString())) {
