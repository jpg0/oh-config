...
with(fluent.withToggle) {
    when(item('Garage_Side_Door_Occupancy').changed().from('CLOSED').to('OPEN')).
    ...
    when(item('Garage_Car_Door_Contact').changed().from('OPEN').to('CLOSED')).
        or(item('Garage_Side_Door_Occupancy').changed().to('CLOSED')).
        then(sendOff().toItem('GarageLights_Switch'));

    when(item('Garage_Car_Door_Contact').changed().from('CLOSED').to('OPEN')).
        or(item('Garage_Side_Door_Occupancy').changed().from('CLOSED').to('OPEN')).
