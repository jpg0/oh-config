try {
    with(require('ohj').fluent.withToggle) {
        ...
        when(cron("0 40 4 ? * 1/7 *")).then(sendOff().toItem("OutdoorWater_Switch_4", inGroup("Irrigation")));
        when(cron("0 20 20 ? * 1/7 *")) .then(sendOn(). toItem("OutdoorWater_Switch_4", inGroup("Irrigation")));
        when(cron("0 40 20 ? * 1/7 *")).then(sendOff().toItem("OutdoorWater_Switch_4", inGroup("Irrigation")));
