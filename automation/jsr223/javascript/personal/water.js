try {
    with(require('ohj').fluent.withToggle) {
        ...
        when(cron("0 0 06 ? * 1/2 *")) .then(sendOn(). toItem("FrontGardenIrrigation_Switch", inGroup("Irrigation")));
        when(cron("0 10 06 ? * 1/2 *")).then(sendOff().toItem("FrontGardenIrrigation_Switch", inGroup("Irrigation")));
